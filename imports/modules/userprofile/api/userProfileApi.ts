...
import { userProfileSch, IUserProfile } from './userProfileSch';
import { userprofileData } from '/imports/libs/getUser';
import { IMeteorError } from '/imports/typings/BoilerplateDefaultTypings';

// endregion

class UserProfileApi extends OfflineBaseApi<IUserProfile> {
	constructor() {
	...
	verifyCode(code: string, email: string, callback = (e: IMeteorError, r: any) => {}) {
		return this.callMethod('verifyCode', code, email, callback);
	}

	resendCode(email: string, callback = (e: IMeteorError, r: any) => {}) {
		return this.callMethod('resendCode', email, callback);
	}
}

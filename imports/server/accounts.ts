...
Meteor.startup(() => {
	...
	Accounts.emailTemplates.verifyEmail.html = async (user, url) => {
		const urlWithoutHash = url.replace('#/', '');
		const userData = (await userprofileServerApi.getCollectionInstance().findOneAsync({ _id: user._id })) || {};
		const email =
			`${
		...
		return await getHTMLEmailTemplate('Confirmação do cadastro', email, footer);
	};
	...
	Accounts.emailTemplates.enrollAccount.html = async (user, url) => {
		const urlWithoutHash = url.replace('#/', '');
		const userData = (await userprofileServerApi.getCollectionInstance().findOneAsync({ _id: user._id })) || {};
		...
		const email =
			...
			'<p><br/>Equipe <b>MeteorReactBase-MUI</b></p>';
		const footer = `Essa mensagem foi gerada automaticamente!`;
		return await getHTMLEmailTemplate('Conclua o seu cadastro', email, footer);
	};
	...
	Accounts.emailTemplates.resetPassword.html = async (user, url) => {
		...
		const email =
			...
			'<p><br/>Equipe <b>MeteorReactBase-MUI</b></p>';
		const footer = `Essa mensagem foi gerada automaticamente!`;
		return await getHTMLEmailTemplate('Alteração da senha atual', email, footer);
	};
	...
	Accounts.validateLoginAttempt(async ({ user, allowed }: { user: Meteor.User; allowed: boolean }) => {
		...
		if (userProfile && userProfile.status === 'disabled') {
			throw new Meteor.Error('user-disabled', 'Sua conta ainda não foi ativada. Verifique seu e-mail.');
		}

...
export const SysSelectField: React.FC<ISysSelectFieldProps> = ({
	...
	const currentOptions = (options && options.length > 0) ? options : (refObject?.current?.options?.length ? refObject.current.options : (schema?.options?.(controllerSysForm?.docId ? {} : (controllerSysForm as any)?.initialDefaultValues) || []));
	...
	const [optionsState, setOptionsState] = useState<Array<IOption> | undefined>(currentOptions);
	...
	return (
		<Collapse in={!!visibleState && (optionsState?.length !== 0 || !!schema?.options)} unmountOnExit sx={{ width: '100%' }}>
			<FormControl error={!!errorState} sx={sxMap?.container}>
				<SysLabelView
					...
					<Select
						...
						renderValue={(val) => {
							const selectedOption = optionsState?.find((option) => option.value === val);
							if (!hasValue(val)) {
								return (
						...
						{optionsState?.length === 0 ? (
							...
							optionsState?.map((option: IOption) => (

...
interface ISysTabs {
	...
	textColor?: 'secondary' | 'inherit' | 'primary' | 'neutral' | undefined;
	/** Cor do indicador de seleção. */
	indicatorColor?: 'secondary' | 'primary' | 'neutral' | undefined;
	...
	return (
		<Container orientation={orientation} sx={sxMap?.container}>
			<Tabs
				...
				textColor={textColor === 'neutral' ? 'inherit' : textColor}
				indicatorColor={indicatorColor === 'neutral' ? 'secondary' : indicatorColor}
				variant={variant}
				allowScrollButtonsMobile={allowScrollButtonsMobile}
				orientation={orientation}
				sysTextColor={textColor}
				sysIndicatorColor={indicatorColor}
				sx={sxMap?.tabs}>
				{abas.map((aba) => (
					<Tab
						label={aba.label}

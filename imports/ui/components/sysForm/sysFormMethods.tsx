	...
	public static setRefComponent = ({
		...
		try {
			...
			if (path.length === 1) {
				...
				const existingRef = mainRef[name] as MutableRefObject<ISysFormComponentRef> | undefined;

				// Se já existir no registro, herdamos o valor e a visibilidade atuais
				componentRef.current = {
					...existingRef?.current,
					...componentRef.current,
					value: componentRef.current.value || initialDefaultValues?.[name] || existingRef?.current?.value,
					isVisible: existingRef?.current?.hasOwnProperty('isVisible')
						? existingRef.current.isVisible
						: schema[name].visibilityFunction?.(initialDefaultValues) ?? true,
					schema: schema[name],
					options: (existingRef?.current?.options && existingRef.current.options.length > 0)
						? existingRef.current.options
						: schema[name].options?.(initialDefaultValues)
				};
		...
		try {
			...
			for (const key in schema) {
				...
				if (!subSchema) {
					...
					if (ref) docValues[key] = ref.current?.value;
				} else {
					const subDoc = doc[key] as IDocRef;
					if (subDoc) docValues[key] = SysFormMethods.getDocValues(subDoc, subSchema);
				}
			}

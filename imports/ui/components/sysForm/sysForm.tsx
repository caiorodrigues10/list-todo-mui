...
const SysForm: ForwardRefRenderFunction<ISysFormRef, ISysForm> = (
	...
	const onChangeComponentValue = useCallback(
		({ refComponent, value }: { refComponent: MutableRefObject<ISysFormComponentRef>; value: any }) => {
			try {
				...
				if (refButton.current?.current?.disabled !== !checkIfAllRequiredFieldsAreFilled(newDoc))
					refButton.current?.current?.setDisabled?.(!checkIfAllRequiredFieldsAreFilled(newDoc));
	...
	const providerValue: ISysFormContext = useMemo(
		...
		[loading, disabled, mode, doc._id, doc.id, setRefComponent, onChangeComponentValue, setInteractiveMethods, setButtonRef]
	);
